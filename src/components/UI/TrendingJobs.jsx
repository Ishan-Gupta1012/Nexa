...
const JobCard = ({ title, field, type }) => {
  ...
  return (
    <a
      href="#"
      className="block group bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-xl transition-all duration-300 hover:-translate-y-1 hover:bg-white/10 hover:border-cyan-400/50"
    >
      <div className="flex justify-between items-start">
        <div>
          <p
            className={`text-xs font-bold uppercase tracking-wider ${typeColor}`}
          >
            {field}
          </p>
  ...
  const getPersonalizedJobs = async (skills) => {
    ...
    try {
      const prompt = `
        ...
        { "jobs": [ { "title": "string", "field": "string", "type": "string" } ] }
      `;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 lg:col-span-1 flex items-center justify-center text-center bg-white/5 backdrop-blur-md border-2 border-dashed border-white/10 p-4 rounded-xl">
            <p className="text-gray-400">
              More roles and regional data coming soon!
            </p>
          </div>
        </div>
      )}
